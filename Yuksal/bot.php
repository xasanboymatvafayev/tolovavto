...
bot('sendMessage',['chat_id'=>$id,'text'=>"⛔ Botdan foydalanish uchun kanallarga obuna bo'ling:",'parse_mode'=>'html','reply_markup'=>json_encode(['inline_keyboard'=>array_chunk($button,1)])]);
exit;
} else return true;
} else return true;
}
...
[['text'=>"✅ Tekshirish",'callback_data'=>"check_pay=".$active['amount']."=".$active['used_order']]],
...
// Xuddi shu summa boshqa pending da
...
$parts=explode("=",$data);
$amount_c=$parts[1]; $order_c=$parts[2];
...
$i++;
if($us['status']=="waiting") $icon="🔄";
elseif($us['status']=="confirm") $icon="✅";
else $icon="⛔";
...
$result2=mysqli_query($connect,"SELECT * FROM `shops` WHERE `user_id`='$cid'");
$has=mysqli_num_rows($result2)>0;
$txt=$has?"🏪 <b>Kassalaringiz:</b>":"⚠️ Kassalar mavjud emas!";
if($edit) bot('editmessagetext',['chat_id'=>$cid,'message_id'=>$mid,'text'=>$txt,'parse_mode'=>'html','reply_markup'=>$kassalar]);
else bot('sendmessage',['chat_id'=>$cid,'text'=>$txt,'parse_mode'=>'html','reply_markup'=>$kassalar]);
}
...
// Foydalanuvchi "Kassa ulash" bosdi
if(mb_stripos($data,"req_connect=")!==false){
$parts=explode("=",$data);
$shop_id_r=$parts[1]; $user_id_r=$parts[2];
...
$key2=[];
while($sh=mysqli_fetch_assoc($shops_r)){
$key2[]=[['text'=>"🏪 ".base64_decode($sh['shop_name']),'callback_data'=>"do_connect=".$sh['shop_id']."=$target"]];
}
mysqli_query($connect,"UPDATE users SET step='null' WHERE user_id='$cid'");
...
$parts=explode("=",$data);
$shop_id_ac=$parts[1]; $user_id_ac=$parts[2];
$shops_r=mysqli_query($connect,"SELECT * FROM shops WHERE shop_id='$shop_id_ac'");
$key2=[];
while($sh=mysqli_fetch_assoc($shops_r)){
$key2[]=[['text'=>"🏪 ".base64_decode($sh['shop_name']),'callback_data'=>"do_connect=$shop_id_ac=$user_id_ac"]];
}
...
$parts=explode("=",$data);
$shop_id_dc=$parts[1]; $user_id_dc=$parts[2];
...
bot('sendMessage',['chat_id'=>$user_id_dc,'text'=>"✅ <b>Kassangiz ulandi! Endi to'lovlar avtomatik qabul qilinadi.</b>",'parse_mode'=>'html']);
}
...
// Oylik to'lov — admin uchun 0, foydalanuvchi uchun 20000
if(mb_stripos($data,"kassa_payment=")!==false){
$parts=explode("=",$data);
$shop_id_p=$parts[1]; $set_id_p=$parts[2];
...
if($data=="add_kassa"){
bot('deletemessage',['chat_id'=>$cid,'message_id'=>$mid]);
bot('sendmessage',['chat_id'=>$cid,'text'=>"🛍️ <b>Yangi kassa qoʻshish!</b>\n\n<i>Kassa nomini yuboring:</i>",'parse_mode'=>'html','reply_markup'=>$back]);
...
bot('sendmessage',['chat_id'=>$cid,'text'=>"✅ <b>Manzil qabul qilindi!</b>\n\n<i>Kassa haqida ma'lumot kiriting:</i>",'parse_mode'=>'html','reply_markup'=>$back]);
mysqli_query($connect,"UPDATE users SET step='add_kassa_info-$name-$text' WHERE user_id='$cid'");
}else{
bot('sendMessage',['chat_id'=>$cid,'text'=>"⚠️ <b>Noto'g'ri format!</b>\n\n<i>@username yoki domen kiriting</i>",'parse_mode'=>'html']); exit;
}
}
}
...
$name=$parts2[1]; $address=$parts2[2];
if($text=="⏪ Ortga"){ mysqli_query($connect,"UPDATE users SET step='null' WHERE user_id='$cid'"); }
else{
...
if($ch){
bot('sendMessage',['chat_id'=>$cid,'text'=>"<b>📑 Foydalanuvchi!</b>\n\nID: <code>".$ch['user_id']."</code>\nBalans: <b>".$ch['balance']."</b> soʻm",'parse_mode'=>"HTML",'reply_markup'=>json_encode(['inline_keyboard'=>[[['text'=>"➕ Pul qoʻshish",'callback_data'=>"pul=plus=$text"],['text'=>"➖ Pul ayirish",'callback_data'=>"pul=minus=$text"]]]])]);
}else{
...
if(!$ch) $ch=mysqli_fetch_assoc(mysqli_query($connect,"SELECT * FROM users WHERE id='$id'"));
$c=$type=="plus"?$ch['balance']+$text:$ch['balance']-$text;
mysqli_query($connect,"UPDATE users SET balance='$c' WHERE user_id='".$ch['user_id']."'");
